...
| Session | Beklenen Durum | Kontrol |
|---|---|---|
| Session 0 | Servis oturumu → yalnızca `SYSTEM` olmalı | Beklenmedik kullanıcı hesabı var mı? |
| Session 1+ | Etkileşimli kullanıcı oturumu | Hangi kullanıcılar oturum açmış? |
...
> **Önemli:** Domain hesapları `hashdump` çıktısında yer almaz. Bunlar için:
...
Elde edilen hash değerleri Hashcat veya John the Ripper ile kırılabilir. `pycryptodome` kütüphanesi kurulu olmalıdır.
